function R_delta = fcn_AVAR_RDeltaRandomWalk(random_walk_coefficient,...
                   list_of_correlation_intervals,lag,sampling_interval)
%% fcn_AVAR_RDeltaRandomWalk
...
%   'list_of_correlation_intervals' for random walk.
%
% FORMAT:
%   R_delta = fcn_AVAR_RDeltaWhiteNoise(random_walk_coefficient,...
%             list_of_correlation_intervals,lag,sampling_interval)
%
% INPUTS:
...
for i = 1:numel(list_of_correlation_intervals)
    ...
    if lag < correlation_interval
        R_delta(i) = (power_spectral_density*sampling_interval)*(4*correlation_interval^3 - 6*correlation_interval*lag^2 + 3*lag^3)/...
                     (6*correlation_interval^2);
    elseif lag < 2*correlation_interval
        R_delta(i) = (power_spectral_density*sampling_interval)*(2*correlation_interval - lag)^3/...
                     (6*correlation_interval^2);
    else
        R_delta(i) = 0;
